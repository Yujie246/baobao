...
test("实现包含完整页面闭环和统一 Mock 边界", async () => {
  const [app, gateway, fixtures, tts, ttsRoute, importValidation] = await Promise.all([
  ...
  assert.match(app, /aria-label="查看宝宝档案"><CharacterIllustration intent="welcome"[^>]*priority/);
...
test("移动端动效体系覆盖路由、列表、按钮、加载和滚动层级", async () => {
  ...
  assert.match(styles, /@keyframes home-character-idle/);
  assert.match(styles, /\.baby-avatar\{position:absolute;[^}]*width:96px;height:96px/);
  ...
  assert.match(styles, /\.home-file-list article button\{width:44px;height:44px/);
